...
=============================================================================*/
    ...
    IF NOT EXISTS (SELECT 1 FROM audit.log_control WHERE validated = FALSE) THEN
        RETURN '✅ All policies are up to date.';
    END IF;
    ...
    LOOP
        ...
        IF r_schema.schema_name = 'audit' THEN
            RAISE WARNING '⚠️ Security Alert: Policy for schema "audit" ignored.';
        ...
        -- Feedback
        IF v_events = '' THEN
            RAISE NOTICE '❌ Audit removed for schema: %', r_schema.schema_name;
        ELSE
            RAISE NOTICE '✨ Audit active (%s) for schema: %', v_events, r_schema.schema_name;
        END IF;
    ...
    RETURN format('🚀 Policies successfully applied to %s tables.', v_counter);
END;
$function$;
